...
- **Receiver Stub Kit** (`docs/learnforge_receiver_stub_kit/`) — a SEPARATE documented patch for the `learnforge-core` repo (NOT implemented inside Radar): `README.md`, `app/api/course-generation-jobs/route.ts` (POST: HMAC verify → Zod validate → forbidden-content guard → idempotency → persist `accepted` → `TODO(generation)` seam → 202), `[jobId]/route.ts` (GET status), `lib/radar/verify.ts` (constant-time HMAC over raw body), `lib/radar/brief.ts` (Zod CourseBriefV2 + `assertNoContentFields`), `prisma/course_jobs.prisma.snippet`, `verify_local.sh` smoke test. Signing verified byte-identical across Radar-Python / openssl / Node-crypto. Kit generates NO syllabus/modules — flips upstream 404 → 202 to unblock the handshake. Referenced from the V2 contract doc §6.
