...
//
// Demand-source attribution is REQUIRED: source.provider and source.source_title
// must be present and non-empty (min length 1).
...
export const SourceEvidence = z
  .object({
    provider: z.string().min(1), // required demand-source attribution
    source_url: z.string().nullable().optional(),
    source_title: z.string().min(1), // required demand-source attribution
    observed_at: z.string().nullable().optional(),
  })
  .strict();

export const DemandEvidence = z
  .object({
...
export const AudienceProfile = z
  .object({
...
export const CommercialHypothesis = z
  .object({
...
export const GenerationConstraints = z
  .object({
